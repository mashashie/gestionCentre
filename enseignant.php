        ...
        if (isset($_POST['first'],$_POST['last'],$_POST['ville'],$_POST['mail'],$_POST['date_de_naissance'],$_POST['diplome'],$_POST['matiere'],$_POST['sexe'],$_POST['age'],$_POST['naissance']) AND !empty($_POST['first']) AND !empty($_POST['last']) AND !empty($_POST['mail'])  AND !empty($_POST['diplome']) AND !empty($_POST['matiere']) AND !empty($_POST['sexe']) AND !empty($_POST['age']) AND !empty($_POST['pseudo']) AND !empty($_POST['pass']) AND !empty($_POST['naissance'])AND !empty($_POST['date_de_naissance'])) {
          ...
          if($mailexist == 0){
            if(($age>=25) AND ($age<60)){
              ...
              $key = 0;
              $key = mt_rand(0,100);
                 $insertmbr = $bdd->prepare("INSERT INTO enseignant(pseudo,session,first,last,email,diplome,ville,filiere,age,autre,naissance,lieu,sexe,unique_id,pass,date_en) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())");
  ...
  <section id="container" class="">
    ...
    <option>DECEMBRE</span></option>
      ...
      <div class="input-group">
          ...
          <textarea name="autre" class="form-control" placeholder="entrez un commentaire"></textarea>
        </div><br>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td><?= $bul['lieu']?></td>
                       <td><?= $bul['ville']?></td>
                      <td><?= $bul['date_en']?></td>
                      <td><a onclick="return(confirm('voulez-vous vraiment supprimer un enseignant ? attention cette action est irreversible'))" href="enseignant.php?del=<?=$bul['id']?>"> <i class=" btn-sm btn-danger fa fa-times-circle"></i></a></td>
                    </tr>
                  </tbody>
            <?php } ?>
                </table>
